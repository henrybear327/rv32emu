...
#define HUMAN_READABLE_OUTPUT 0
...
int main()
{
    ...
    std::cout << "Worst:   " << formatTime(freeTimes[freeCount - 1])
              ...
              << std::endl;
#else 
    // malloc count, free count
    // alloc (ns) p50, p90, p99, worst
    // free (ns) p50, p90, p99, worst
    std::cout << mallocCount 
    << "," << freeCount 

    << "," << allocTimes[size_t((float) mallocCount * 0.50f)]
    << "," << allocTimes[size_t((float) mallocCount * 0.90f)]
    << "," << allocTimes[size_t((float) mallocCount * 0.99f)]
    << "," << allocTimes[mallocCount - 1] 

    << "," << freeTimes[size_t((float) freeCount * 0.50f)]
    << "," << freeTimes[size_t((float) freeCount * 0.90f)]
    << "," << freeTimes[size_t((float) freeCount * 0.99f)]
    << "," << freeTimes[freeCount - 1] << std::endl;
#endif 
}
